...
uint32_t apply_sobel(uint32_t img[SIZE][SIZE], int32_t filter[2][SIZE][SIZE]) {
#pragma HLS inline
    int32_t Gx = 0, Gy = 0;
...
#pragma HLS UNROLL
    ...
    return (uint32_t)hls::sqrt((double)(Gx * Gx + Gy * Gy));
}
...
#pragma HLS INTERFACE s_axilite port=return bundle=CONTROL_BUS
    ...
    uint32_t LINE_BUFFER[2][MAX_WIDTH], WINDOW_BUFFER[SIZE][SIZE];
    int32_t FILTER[2][SIZE][SIZE] = {{{1, 0, -1},
                                      {2, 0, -2},
                                      {1, 0, -1}},
                                     {{1, 2, 1},
                                      {0, 0, 0},
